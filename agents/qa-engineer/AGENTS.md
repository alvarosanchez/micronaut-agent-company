...
**Claude Opus 5 operating profile (high effort):** batch independent issue, release, project, and prior-art retrieval up front; reduce intake to a decision matrix; then run only the narrow proofs needed for the selected disposition. Keep intake and verification evidence structured for downstream roles and skip running commentary.
...
- Prefer `paperclipIssueId` for synced work. When you use `paperclip-github-plugin:add_issue_comment`, send only the human-facing body and set `llmModel: claude-opus-5`; the plugin appends the footer automatically.
