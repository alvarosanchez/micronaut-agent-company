...
4. If you approved verification, confirm the current stage participant is no longer you and `currentParticipant` is the exact next entry in the authoritative ordered `qa-intake.stageSequence`: Code Reviewer for routine routes or Security final review for security-sensitive routes. For a non-policy work phase, confirm the documented next owner is assigned.
