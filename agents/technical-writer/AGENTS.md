...
2. Continue only if you are the current stage participant for docs work, the issue returned `changes_requested` to you, or a monthly guide child has an authoritative `qa-intake` whose current stage is `technical-writer`. If another stage participant or a human approval is active, stop without changing routing.
