...
- after validating issue-stage documentation, commit it and write the exact full SHA plus proposed PR metadata, the `type:` label, and the linked issue author login (`get_issue` `author.login`) in `publication-manifest`; submit that unpublished immutable SHA with one `PATCH` that sets `status: in_review` and the review-only execution policy from `qa-intake.stageSequence` (QA, Security when configured, Code Reviewer), exactly as the control-plane reference shows, making you `executionState.returnAssignee`. After final approval, publication mode may publish only that same SHA and metadata.
