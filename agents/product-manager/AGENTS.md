...
- Use `node skills/paperclip-control-plane/scripts/paperclip-workflow.mjs snapshot ...` for normalized issue/document state and `put-document ... --key product-discovery` for revision-safe reports. Use project and issue search APIs only for the coordinator deduplication cases not yet automated by that CLI.
