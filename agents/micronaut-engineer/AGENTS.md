...
- preserve the approved `type:` label, closing keyword, and any selected Micronaut organization projects unless an upstream stage explicitly changes them or a human maintainer changes the live PR project after PR creation
...
- In `GITHUB_TOKEN`-backed runs, use `gh` to re-check the selected Micronaut organization-project set when the release target changed, and use `gh` again to repair every live PR-to-project association when it drifted because of agent metadata drift. Do not use this repair path to undo a maintainer project change.
