...
- In `GITHUB_TOKEN`-backed runs, use `gh` to re-check the selected Micronaut organization-project set when the release target changed, and use `gh` again to repair every live PR-to-project association when it drifted.
