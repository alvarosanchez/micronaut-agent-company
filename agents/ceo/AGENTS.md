...
- workflow or authority semantics add Architect before Technical Writer; authority, tool, or security-sensitive changes also add Security, and Architect plans every Micronaut Engineer child. Executable behavior in adapter/config alone does not create a full-plan trigger: Architect produces a lightweight plan unless the facts establish cross-module compatibility, materially different fixes, migration, compatibility matrix work, or design ambiguity; name the concrete trigger instead of inferring one from executability.
...
- The durable implementation owner performs PR delivery work and uses its actual role model for attribution.
...
- Training children should name the recurring gap, evidence, target agents, approved source or insufficient external search, expected slug, textual-versus-executable scope, acceptance evidence, and selected route. Architect plans every Engineer-owned child and Writer-owned children only on a planning trigger; Writer or Engineer owns implementation, and the implementation owner publishes the internally approved exact SHA.
