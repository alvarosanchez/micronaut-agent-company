...
- workflow or authority semantics add Architect before Technical Writer; authority, tool, or security-sensitive changes also add Security, and package/plugin architecture or compatibility adds Architect before Micronaut Engineer
...
- If GitHub Sync reopens a PR-based issue for actionable CI/review feedback, route it to the durable follow-through owner. Source/test/dependency/build changes re-enter Micronaut Engineer -> QA -> Security -> Code Reviewer; prose docs re-enter Technical Writer -> QA -> Code Reviewer; executable/security docs add Security; design changes add Architect before the owner.
