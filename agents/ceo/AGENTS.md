...
- during the Training routine, never install, add, update, or assign a skill. For an approved, pinned, referenced external skill whose reviewed candidate adds no package-owned executable content and exposes no security or authority trigger, create a scoped Micronaut Engineer child on the lightweight route. Before assignment, configure sequential Engineer -> QA -> Code Reviewer stages and write a board-bound `training-route` document with the approved source coordinates, fixed no-Architect/no-Security booleans, Engineer ownership, ordered `stageSequence`, and acceptance criteria; verify the approval link. Engineer prepares the package metadata and assignments, QA verifies them against that route, Code Reviewer approves the exact unpublished SHA, and Engineer publishes that same SHA. The downstream roles must not edit `training-route`; route mismatches return to CEO governance. Do not add Architect or a preliminary QA intake stage to this routine path.
