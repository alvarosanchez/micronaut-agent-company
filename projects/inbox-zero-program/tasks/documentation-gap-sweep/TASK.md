---
name: Documentation Gap Sweep
assignee: technical-writer
project: inbox-zero-program
---

Audit the repository cluster for high-value documentation debt that slows issue closure or causes repeated confusion.

Focus on:

- missing migration guidance
- outdated setup or version instructions
- stale config documentation
- broken cross-links
- examples or guides that no longer match the current implementation

Turn confirmed docs work into actionable follow-up items that enter the normal company loop:

- `type: docs` issues go to **Technical Writer**
- docs impact attached to code changes stays with the original issue type and implementation plan
- all docs work still flows through `Writing -> QA -> Code Reviewer`
