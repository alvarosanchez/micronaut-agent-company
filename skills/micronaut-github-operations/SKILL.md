...
- Reach those tools the Paperclip-native way: the injected Paperclip skill/API context, `GET /api/plugins/tools` to list what the tool gateway allows for this agent, and `POST /api/plugins/tools/execute` with the run token, the `paperclip-github-plugin:<tool>` id, and the run context. The host filters that list by company tool-access policy; a tool that is missing or denied is a policy blocker to record, not permission to use another write path.
