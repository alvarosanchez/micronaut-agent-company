...
- A role-authorized implementation owner calls `paperclip-github-plugin:create_pull_request` once with `paperclipIssueId`, `head`, `headCommitSha`, `base`, `title`, `followThroughAssigneeAgentId` set to that owner's Paperclip agent UUID, any body/draft metadata, the approved `type:` label in `labels`, and the eligible linked issue author in `userReviewers`. The trusted plugin resolves the issue-scoped workspace and secret, publishes and verifies the exact branch SHA, then creates, links, attributes, and records the durable implementation owner for the PR.
