...
- Reach those tools the Paperclip-native way: `GET /api/plugins/tools` lists what the tool gateway allows for this agent and `POST /api/plugins/tools/execute` runs one through the tool gateway with the run token, the `paperclip-github-plugin:<tool>` id, `parameters`, and a `runContext` of `agentId`, `runId`, `companyId`, `projectId`. Use the `paperclip-control-plane` helper for both, `node <paperclip-control-plane-skill-directory>/scripts/plugin-tool.mjs list` and `... call paperclip-github-plugin:<tool> --params '<json>'`, instead of rebuilding that request by hand. The host filters the list by company tool-access policy; a tool that is missing or denied is a policy blocker to record, not permission to use another write path.
- Do not use `gh` as a fallback for GitHub API reads/writes, inspect credentials, run `git push`, or search for a GitHub token. The same applies to direct `api.github.com` calls (curl, scripts): they run unauthenticated, hit rate limits, and leak the host address. Read the default branch with `git symbolic-ref refs/remotes/origin/HEAD`.
...
- A role-authorized implementation owner calls `paperclip-github-plugin:create_pull_request` once with `paperclipIssueId`, `head`, `headCommitSha`, `base`, `title`, `followThroughAssigneeAgentId` set to that owner's Paperclip agent UUID, any body/draft metadata, the approved `type:` label in `labels`, and the eligible `reviewerCandidates` reported by `get_issue` (the issue author and repository members who commented on the issue, minus the PR author) in `userReviewers`. Labels and reviewer requests are applied after the PR exists and a failure comes back as `warnings` without undoing the PR: repair a label warning with `paperclip-github-plugin:update_pull_request` `labels` set to the exact approved replacement set, and a reviewer warning with `paperclip-github-plugin:request_pull_request_reviewers`. The trusted plugin resolves the issue-scoped workspace and secret, publishes and verifies the exact branch SHA, then creates, links, attributes, and records the durable implementation owner for the PR.
...
- The company metric endpoint is a native plugin JSON route with agent auth, not a plugin-tool call or webhook. Authenticate the native metric route with `Authorization: Bearer ${PAPERCLIP_API_KEY}`; `PAPERCLIP_API_KEY` is the run-minted agent token (48h default TTL) that Paperclip injects into every authenticated run, never a static key.
...
- Do not clear the durable owner merely because the PR is currently clean, green, mergeable, or parked on the human owner for maintainer review. Later CI failures, conflicts, or review threads must return to the recorded implementation owner.
