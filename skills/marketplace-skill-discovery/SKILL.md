...
- Approval does not transfer implementation authority to CEO. A pinned referenced candidate with no package-owned executable content and no security, authority, provenance, or integration trigger uses the lightweight Architect plan -> Engineer -> QA -> Reviewer -> Engineer-publication route. Name any trigger that requires a QA-intake child instead of silently adding QA intake to every external skill.
- If no marketplace candidate is suitable, create the same scoped QA-assigned child for a company-owned skill proposal. Architect plans every Engineer-owned skill child (lightweight unless QA identifies a full-plan trigger) and Writer-owned content only when QA identifies a real planning trigger; Writer or Engineer remains the implementation and PR owner.
