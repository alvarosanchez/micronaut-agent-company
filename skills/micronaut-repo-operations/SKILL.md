...
- Plugin tools arrive through Paperclip's tool gateway (`GET /api/plugins/tools`, `POST /api/plugins/tools/execute` with the run token) filtered by company tool-access policy. Runs may resume persistent ACP session state instead of a fresh process, so re-read the issue state and `PAPERCLIP_WORKSPACE_*` environment on every wake instead of trusting remembered context or working directory.
...
- Pre-delivery steps are `TODO` handoffs. The delivery owner creates the review-only execution policy when it submits the SHA; `executionState.currentParticipant` resolves the active stage, `executionState.returnAssignee` (the delivery owner) receives `changes_requested`, and review stays `in_review` until the route advances.
