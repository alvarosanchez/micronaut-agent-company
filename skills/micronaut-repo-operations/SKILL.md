...
- For `paperclip-github-plugin:add_issue_comment` and `paperclip-github-plugin:reply_to_review_thread`, send only the human-facing body and set `llmModel` to your exact runtime model id: `gpt-5.5-pro` for agents configured with `high` or `xhigh` reasoning, and `gpt-5.5` for agents configured with `medium` reasoning. The plugin appends the same Markdown footer automatically.
