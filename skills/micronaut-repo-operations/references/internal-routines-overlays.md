...
- for Technical Writer guide routines, keep the parent routine issue as a coordinator only: it creates one Paperclip child issue or subtask per affected project when the project exists in Paperclip, puts each child in the actual corresponding project, sets `parentId` to the routine issue when supported, sets assignee to QA Engineer (`qa-engineer`), and requires normal QA intake to write `qa-intake` before Writer implementation with exact `stageSequence` `qa-engineer`, `technical-writer`, `qa-engineer`, `code-reviewer`, `technical-writer-publication`; the coordinator then stops instead of opening or updating PRs
