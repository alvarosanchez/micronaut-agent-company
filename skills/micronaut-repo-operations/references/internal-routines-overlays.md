...
- routine-created work follows the QA-selected route: routine bugs and compatible dependency upgrades skip Architect; architectural/migration work adds Architect; security-sensitive work uses Security pre-triage and final review; prose docs use Writer -> QA -> Reviewer; executable/security docs add Security. The issue is not complete until a linked PR exists or the child records a no-diff/no-PR decision or named blocker
