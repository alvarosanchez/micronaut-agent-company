...
Every `stageSequence` entry is an imported agent slug. The sequence splits at `deliveryOwner` into two mechanisms. Entries up to and including the delivery owner form the **handoff chain**: each owner finishes its artifact and passes the issue to the next entry with a non-policy `TODO` assignment plus a next-action comment (QA intake -> Security pre-triage -> Architect -> delivery owner), and sends work back the same way, `TODO` to the previous entry naming the mismatch. Entries after the delivery owner form the **review chain**: when the delivery owner submits the immutable SHA it creates the review-only execution policy with exactly those entries as separate sequential review stages (`qa-engineer`, then `security-engineer` only when `securityFinalReviewRequired: true`, then `code-reviewer`) in the same `PATCH` that moves the issue to `in_review` (exact body in `workflow-control-plane.md`). The host records the delivery owner as `executionState.returnAssignee`, so every `changes_requested` returns to it, and never selects the return assignee as a stage participant. That is why no pre-delivery step is a policy stage and why QA does not create the execution policy at intake: an intake-created policy whose later stages name the intake owner or the delivery owner fails with `422 No eligible review participant is configured for this issue` at the first advance into such a stage. Publication is not a policy stage: after Code Reviewer completes the final stage, it creates the documented publication-only non-policy `TODO` handoff to `followThroughOwner`, and runtime dispatch wakes that owner.
...
QA records the selected sequence in `qa-intake` together with the plan depth, why optional Security stages are present or absent, and, for Writer-owned work, why Architect is present or absent, then hands the issue `TODO` to the next handoff-chain entry; QA does not create the execution policy. Implementation may escalate an exposed design gap back to Architect; unresolved behavior, compatibility, or security questions are escalations, never permission to improvise.
