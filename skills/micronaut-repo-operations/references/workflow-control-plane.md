...
- Synced GitHub issues use an issue `executionPolicy` only for the post-delivery review chain. QA intake, Security pre-triage, Architect planning, and implementation are sequential `TODO` assignment handoffs; the delivery owner works as the assignee in `in_progress`. Inside an active stage, Paperclip is the routing mechanism.
- Use review stages for genuine post-delivery sign-off gates: QA verification, Security Engineer final review when `securityFinalReviewRequired: true`, and Code Reviewer.
- The delivery owner creates the policy when it submits the immutable SHA, in one call: `PATCH /api/issues/{issueId}` with `{"status":"in_review","executionPolicy":{"mode":"normal","stages":[{"type":"review","participants":[{"type":"agent","agentId":"<qa-engineer>"}]},{"type":"review","participants":[{"type":"agent","agentId":"<security-engineer>"}]},{"type":"review","participants":[{"type":"agent","agentId":"<code-reviewer>"}]}]},"comment":"<submission note: SHA, manifest revision>"}`, omitting the Security stage unless the route requires it. Resolve agent ids by slug from `GET /api/companies/{companyId}/agents`. The host sets `executionState.returnAssignee` to the assignee at that moment (the delivery owner) and `currentParticipant` to the first stage.
- The host never selects the return assignee as a stage participant and answers `422 No eligible review participant is configured for this issue` when a stage has nobody else. Therefore the delivery owner is never a participant, no pre-delivery role is a policy stage, and nobody creates the policy at intake.
- Resubmitting after `changes_requested`: if the reviewed SHA is unchanged (artifact or metadata fix only), `PATCH {"status":"in_review","comment":"..."}` re-pends the same stage with the same reviewer. If the SHA changed, the whole review chain restarts: first `PATCH {"executionPolicy":null}` (the host clears the stale state), then the submission `PATCH` above with a fresh policy. Never use a policy reset to escape a review-rounds escalation: when `currentParticipant` is a human, wait for that human.
...
- Never end a run with the issue still `in_progress` and assigned to you unless your work genuinely continues next heartbeat: the host re-wakes such an issue once (`issue.productive_terminal_continuation_recovery`) and then blocks it as stranded. Hand it off, submit it, or park it (`in_review` with the human owner on `assigneeUserId` for sync-owned closures and healthy PR waits) before finishing.
- A run ends when you stop, and nothing you arm survives it: monitors, timers, watchers, and background processes in the run scratch directory are gone on the next wake, and no wake is scheduled by them. Wait for long commands inside the run with bounded polling (sleep-and-check loops), and if a proof cannot finish within the run budget, write the partial evidence, the exact command and its log path to the durable artifact and hand off or park the issue explicitly. Never finish with "I'll continue when it reports".
...
- Use normal `TODO` assignment for every handoff-chain step (intake-to-pre-triage, pre-triage-to-planning, planning-to-implementation), for sending pre-delivery work back to the previous owner, for publication, and for post-PR follow-through. In those cases, assign the next owner and add a comment that names the reason and next action; Paperclip owns dispatch.
...
- The installed `paperclipai@2026.916.0` runtime in this package still types execution stages as the literal `approvalsNeeded: 1`, so do not rely on a single multi-participant stage for unanimous sign-off. If all listed reviewers must approve in order, model that as separate sequential stages.
...
- Re-entry is effect-based: no repository change returns to the implementation owner only for any required response, then to maintainer wait; routine source, test, dependency, or build changes go Micronaut Engineer -> QA -> Code Reviewer under the PR's existing approved plan (or the lightweight plan recorded in `qa-intake`/`training-route`); routine prose or executable docs go Technical Writer -> QA -> Code Reviewer; behavior-changing executable instructions with no established Security pre-triage trigger set `securityPrecheckRequired: false`, `securityFinalReviewRequired: true`, and go Technical Writer -> QA -> Security final -> Code Reviewer; defined Security triggers add pre-triage before the owner and final Security review after QA; only design-changing requests go Architect -> recorded implementation owner -> applicable gates. A clean rebase with green CI returns to maintainer wait; conflicts or semantic changes rerun the applicable gates. Escalate unresolved behavior, compatibility, or security questions instead of improvising.
...
- If GitHub Sync drops a PR-based issue from `in_review` to `in_progress` but the live PR is open, non-draft, `CLEAN`, all checks pass, and no actionable review state remains, restore `in_review` maintainer wait without waking an agent.
- CEO governs, synthesizes, prioritizes, corrects safe Paperclip routing drift, and creates/assigns scoped children with acceptance criteria, then stops. CEO never branches, edits, commits, pushes, creates or updates PRs, repairs CI, replies to review threads, or performs PR rediscovery. Textual findings route to Technical Writer; executable package/plugin findings route to Micronaut Engineer behind an Architect plan; add Architect for Writer-owned work and Security when QA classification requires them.
...
- Since `2026.916.0` the host refuses an agent-authored update that moves an issue into `in_review` with no review path and answers `invalid_issue_disposition` / `missing: review_path`. The accepted paths are a pending issue-thread interaction, a linked pending approval, `assigneeUserId`, a typed `executionState.currentParticipant`, or a scheduled issue monitor. So parking is a PATCH with `status: in_review`, `assigneeAgentId: null`, and `assigneeUserId` set to the issue's `responsibleUserId` (read it from `GET /api/issues/{issueId}`) — the human who owns maintainer review. Send it without a `comment`, so the PATCH does not wake anyone. This covers both parking states: a green PR waiting on upstream maintainers, and an issue waiting for GitHub Sync to record a closure. Do not arm an issue monitor for GitHub-synced PR state; GitHub Sync owns that wait.
...
- Do not leave agent-assigned non-terminal work stranded. Paperclip v2026.512.0, still true in `paperclipai@2026.916.0`, makes assigned issue creation default to `todo` when status is omitted, while explicit `backlog` still parks work for human review. An assigned `todo` with no prior issue run is dispatched by recovery as a normal assignment wake and reported as `assignmentDispatched`; wait for or repair that wake instead of creating duplicate recovery issues. An assigned `in_progress` should have an active run, queued continuation, liveness recovery state, or recorded next-action hint. Inspect checkout, execution run, liveness, continuation-attempt, watchdog, and `continuation-summary` context before creating duplicate work. If Paperclip's configured liveness recovery surfaces the issue as stranded or moves it to `blocked` with a visible comment, treat that as an operational problem to repair or escalate. Since Paperclip 2026.831 recovery never takes work over automatically: silent-run detection is UI-only, exhausted or unsafe recovery routes to a board-owned action, and agent-initiated assignment to a paused agent is refused (only a board user may assign one), so the repair or escalation is yours to make.
...
- read-only diagnostics before repairing stranded or tangled work: `GET /api/issues/{issueId}/diagnostics/subtree` (hierarchy and rollup), `GET /api/issues/{issueId}/diagnostics/blockers` (dependency graph), and `GET /api/issues/{issueId}/diagnostics/wakes` (why the issue did or did not wake); a diverged worktree is diagnosed by the host recovery card, and its branch reconciliation stays a board-owned action; issue search accepts filter operators, `GET /api/companies/{companyId}/search/extract` returns bulk fields, and the issues list accepts `updatedSince` for incremental scans
- run output contract: the host keeps agent thought text out of automatic comments and summarizes only the final output segment, so write every deliverable to a keyed document or comment explicitly; `ask_user_questions` cards expire when a later comment or plan revision supersedes them
...
- Runs execute in the resolved issue execution workspace, which may be the project primary workspace or a separate durable execution workspace. Paperclip injects `PAPERCLIP_WORKSPACE_*` and `PAPERCLIP_RUNTIME_*` variables for each run; read them instead of assuming a working directory, because ACP persistent sessions may resume prior state rather than start a fresh process.
