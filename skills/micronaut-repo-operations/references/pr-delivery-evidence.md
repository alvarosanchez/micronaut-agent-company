...
- When the selected projects exist and GitHub tooling can apply them, agents should create every live PR-to-project association with `paperclip-github-plugin:add_pull_request_to_project` or its MCP-bridged runtime name instead of only restating the intended boards in prose. Naming a target board in comments, summaries, or PR description prose is not a substitute for the live PR association.
