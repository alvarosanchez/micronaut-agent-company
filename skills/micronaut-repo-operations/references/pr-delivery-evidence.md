...
- Once the target branch is identified for any PR, sync the work branch to it before starting work, editing, committing, opening, creating, or updating the PR: with no own commits `git reset --hard origin/<target>` (Paperclip cuts worktrees from the default branch, so a rebase onto another release line replays history and conflicts), otherwise rebase. If that rebase or merge produces conflicts, record the conflict as a blocker and do not open, create, or update a conflicting PR.
