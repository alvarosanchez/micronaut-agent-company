...
- The implementation owner uploads required PR-visible assets before QA verification with `paperclip-github-plugin:upload_pull_request_asset`, embeds the returned `asset.markdown` in the PR body with `update_pull_request`, and records what each asset proves. QA verifies those assets in `qa-verification`; Code Reviewer only checks that the approved assets remain present and unchanged.
