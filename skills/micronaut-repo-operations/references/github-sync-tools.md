...
- Use the agent tools below for GitHub API reads they cover. Only authorized implementation owners may use their write operations, including organization-project association. Discover and call them through `GET /api/plugins/tools` and `POST /api/plugins/tools/execute` through the tool gateway (body `{tool, parameters, runContext:{agentId, runId, companyId, projectId}}`; use `plugin-tool.mjs` from `paperclip-control-plane` instead of hand-built requests) with the run token; the tool gateway applies the company tool-access policy, so a missing or denied tool is a policy blocker to record.
...
- For an authorized implementation owner, `create_pull_request` publishes the local branch and creates the PR atomically from the agent's perspective. Pass `paperclipIssueId`, the plain local `head` branch, its exact full `headCommitSha`, `base`, `title`, any body/draft metadata, the approved `type:` label in `labels`, and the `get_issue` `reviewerCandidates` logins (issue author and repository members who commented, excluding bots and the PR author) in `userReviewers` in one call. The trusted plugin verifies the local and remote SHA before opening and linking the PR, then applies the labels and reviewer requests and reports them as `labels`, `requestedReviewers`, or `warnings`. A warning leaves the PR open: repair labels with `update_pull_request` `labels` (a replacement set; the PR ends with exactly those labels) and reviewers with `request_pull_request_reviewers`. `get_issue` returns the issue `author`, all `participants`, and the `reviewerCandidates` subset (non-bot owners, members, and collaborators).
...
- PRs from recurring routines remain with their durable implementation owner until CI is green and actionable review feedback is resolved; healthy maintainer wait parks on the human owner. CEO does not own or rediscover those PRs.
...
- QA verifies each local asset and its hash or identity in `publication-manifest`. After exact-SHA internal approval, the implementation owner uploads that same asset, embeds the returned markdown in the approved PR body, and reads the live result back without changing the approved commit.
