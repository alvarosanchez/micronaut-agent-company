...
- For an authorized implementation owner, `create_pull_request` publishes the local branch and creates the PR atomically from the agent's perspective. Pass `paperclipIssueId`, the plain local `head` branch, its exact full `headCommitSha`, `base`, `title`, any body/draft metadata, the approved `type:` label in `labels`, and the linked issue author login in `userReviewers` (when eligible, non-bot, and not the PR author) in one call. The trusted plugin verifies the local and remote SHA before opening and linking the PR, then applies the labels and reviewer requests and reports them as `labels`, `requestedReviewers`, or `warnings`. A warning leaves the PR open: repair labels with `update_pull_request` `labels` (a replacement set; the PR ends with exactly those labels) and reviewers with `request_pull_request_reviewers`. `get_issue` returns the issue `author`.
