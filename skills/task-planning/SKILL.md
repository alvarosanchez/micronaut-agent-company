...
metadata:
  paperclip:
    catalog:
      ...
      sourceRef: "sha256:4fb46a4bcefad4fd46fae48c433ee497112509a8e19fb8a7745ead44d219b498"
---

## Micronaut Agent Company usage notes

Use this catalog skill when a Paperclip issue explicitly needs a structured implementation plan, child task breakdown, blockers, owners, or an approval-ready `plan` issue document. It strengthens the Architect and CEO planning surfaces, but it does not change the company's default issue mode.

For this company:

- The Architect planning stage on a normal delivery issue is **not** the same as Paperclip `workMode: planning`; synced GitHub delivery issues, routine subtasks, PR follow-through issues, and Product Manager-created backlog product issues remain standard work mode unless a human explicitly asks for a planning-only issue.
- Planning must consume QA's release-targeting facts: repository, default branch, latest stable release, next release, SemVer delta, target branch, organization-project set, label, and PR/closure policy.
- When a plan needs human acceptance, update the `plan` document first, then create a `request_confirmation` interaction tied to that plan revision.
- Child issues should represent real dependency or work-breakdown semantics; use `parentId`, `blockParentUntilDone`, and `blockedByIssueIds` deliberately rather than treating parent links as blockers by default.
