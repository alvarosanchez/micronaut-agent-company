...
metadata:
  paperclip:
    catalog:
      skillKey: paperclipai/bundled/paperclip-operations/issue-triage
      catalogId: "paperclipai:bundled:paperclip-operations:issue-triage"
      catalogKey: paperclipai/bundled/paperclip-operations/issue-triage
      catalogKind: bundled
      catalogCategory: paperclip-operations
      catalogPath: catalog/bundled/paperclip-operations/issue-triage
      packageName: "@paperclipai/skills-catalog"
      packageVersion: 0.3.1
      originVersion: 0.3.1
      originHash: "sha256:88dc13560371fb364963782cb4f6eeb4090fcde92ee3774479428ed6b90e11c1"
      sourceRef: "sha256:88dc13560371fb364963782cb4f6eeb4090fcde92ee3774479428ed6b90e11c1"
---

## Micronaut Agent Company usage notes

Use this catalog skill for CEO or manager queue-health passes, productivity-review handling, and stale or blocked Paperclip inbox reviews. It complements `micronaut-repo-operations`; it does not replace QA's GitHub-backed intake and closure rules.

For synced GitHub delivery work:

- Do **not** close a synced GitHub issue with a one-line reason. QA closure paths still require evidence-rich GitHub comments, the correct disposition, and GitHub Sync reconciliation.
- Respect `executionState.currentParticipant`, `returnAssignee`, linked approvals, open issue-thread interactions, blockers, and active checkout/liveness metadata before reassigning or waking work.
- Peer agents should not mutate another agent-owned issue unless the workflow, reporting chain, or active checkout policy explicitly allows it.
- A triage pass should leave a single next action: resume, wake, reassign, unblock, escalate, close through the approved closure path, or intentionally wait.
...
6. **Close** — work is complete, duplicate, or no longer relevant. In generic Paperclip inboxes this can be `done` or `cancelled` with a short reason; in Micronaut synced GitHub delivery work, use QA's evidence-rich GitHub closure path instead of a one-line closure.
