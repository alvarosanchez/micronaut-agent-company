...
Route by the nature of the gap before proposing a skill: a generalizable, multi-step procedure with its own when-to-use logic is skill material; a narrow, agent-specific rule that fits in one sentence belongs in that role's `AGENTS.md` through the monthly CEO self-improvement route, not in a new skill; when both apply, propose the skill plus a one-line pointer in the role file. Every proposal names three to five past executions the new skill must still pass, and drops or rewords any guidance that would have blocked a past success without a clear reason to retain it (a safety or compatibility constraint is such a reason).
