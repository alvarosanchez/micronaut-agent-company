...
- the tool gateway actually serves this company's plugin tools, with least-privilege bindings: call `GET /api/plugins/tools` with your own run token and confirm your own effective list is exactly the read-only baseline (`search_repository_items`, `get_issue`, `list_issue_comments`, `get_pull_request`, `list_pull_request_files`, `get_pull_request_checks`, `list_pull_request_review_threads`, `list_organization_projects`), because CEO is read-only for GitHub Sync governance. Any `paperclip-github-plugin:*` write tool in your own list is a privilege gap to report. Then confirm with the operator that the delivery profiles are bound per agent rather than at company scope: `update_issue`, `add_issue_comment`, and `assign_to_current_user` for QA Engineer and Product Manager, and `create_pull_request`, `update_pull_request`, `request_pull_request_reviewers`, `reply_to_review_thread`, `resolve_review_thread`, `unresolve_review_thread`, `add_pull_request_to_project`, and `link_github_item` for Micronaut Engineer and Technical Writer only, with Architect, Security Engineer, and Code Reviewer left on the baseline. The gateway is fail-closed on Paperclip >= 2026.831: with no active tool-access profile bound to the agent the list comes back empty and `POST /api/plugins/tools/execute` returns `deny_default`. An empty or short list means the deployment is missing an active tool-access profile with `tool_name` include entries for those `paperclip-github-plugin:*` tools; record that as a blocking operator gap with the exact missing tool ids, and do not treat it as a transient error or fall back to `gh`
