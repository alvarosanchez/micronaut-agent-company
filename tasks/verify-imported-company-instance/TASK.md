...
- `Weekly Product Discovery` exists as an active routine owned by `product-manager`
- `Weekly User Guide Review` exists as an active routine owned by `technical-writer`
- `Weekly Guide Topic Discovery` exists as an active routine owned by `technical-writer`
...
- the imported company includes the package-adopted Paperclip catalog skills `issue-triage`, `task-planning`, `qa-acceptance`, `github-pr-workflow`, `doc-maintenance`, and `agent-browser`, the expected agents reference those skills in their runtime skill inventory, and the skill bodies are the package-copied catalog bodies plus Micronaut usage notes rather than dynamically appended Skills Store content
