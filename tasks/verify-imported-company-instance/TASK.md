...
- the tool gateway actually serves this company's plugin tools: call `GET /api/plugins/tools` with your own run token and confirm the response lists the `paperclip-github-plugin:*` tools this workflow uses (`search_repository_items`, `get_issue`, `list_issue_comments`, `update_issue`, `add_issue_comment`, `create_pull_request`, `get_pull_request`, `update_pull_request`, `list_pull_request_files`, `get_pull_request_checks`, `list_pull_request_review_threads`, `reply_to_review_thread`, `resolve_review_thread`, `unresolve_review_thread`, `request_pull_request_reviewers`, `list_organization_projects`, `add_pull_request_to_project`, `link_github_item`). The gateway is fail-closed on Paperclip >= 2026.831: with no active tool-access profile bound to the agent the list comes back empty and `POST /api/plugins/tools/execute` returns `deny_default`. An empty or short list means the deployment is missing an active company-scoped tool-access profile with `tool_name` include entries for those `paperclip-github-plugin:*` tools; record that as a blocking operator gap with the exact missing tool ids, and do not treat it as a transient error or fall back to `gh`
- the import used `pauseAutomations: true` and the resume sequence is correct: imported agents carry `pauseReason: "import"` and the routines imported `paused` (Paperclip defaults the flag to `false`, which would have let agents wake on an unverified import). This bootstrap issue is the point where automations are resumed: after the checks above pass, resume the remaining package agents and reactivate the monthly routines, and record which agents and routines you resumed. If the import ran without the flag, record that agents may already have acted on an unverified import and verify their activity before continuing
