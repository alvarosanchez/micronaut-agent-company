...
- inside each project-specific subtask, determine whether a documentation PR is needed; because any such PR is created outside the normal delivery pipeline, open or update that project-repository PR only for evidence-backed documentation fixes, and link any resulting PR to that child issue or subtask
