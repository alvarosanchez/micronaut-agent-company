...
4. **Architect** handles the planning stage for `type: improvement`, `type: enhancement`, `type: breaking`, and `type: dependency-upgrade` work, locks the implementation plan, and records the recommended Micronaut organization project when the best-fit release board is clear.
...
8. **Code Reviewer** handles the final review stage, creates the GitHub PR directly when the work is approved, or verifies an acceptable already-open PR, and links the surviving PR to the recommended Micronaut organization project when the target board is clear and GitHub tooling can apply it.
