...
const QA_ASSIGNMENT_TOOL_PATTERN =
  /paperclip-github-plugin:assign_to_current_user[\s\S]{0,700}gh issue edit[\s\S]{0,180}--add-assignee\s+["'`]?@me["'`]?|gh issue edit[\s\S]{0,180}--add-assignee\s+["'`]?@me["'`]?[\s\S]{0,700}paperclip-github-plugin:assign_to_current_user/i;
...
const REVIEWER_TOOLING_PATTERN =
  /request_pull_request_reviewers[\s\S]{0,700}gh pr edit[\s\S]{0,220}--add-reviewer|gh pr edit[\s\S]{0,220}--add-reviewer[\s\S]{0,700}request_pull_request_reviewers/i;
...
test("QA triage assigns imported GitHub issues to the current GitHub user", async () => {
  ...
  assert.match(
    ...
    "QA instructions must prefer paperclip-github-plugin:assign_to_current_user and fall back to `gh issue edit ... --add-assignee \"@me\"`.",
  );
});
...
test("Code Reviewer requests the linked GitHub issue creator as reviewer on created PRs", async () => {
  ...
  assert.match(
    ...
    "Code Reviewer instructions must describe reviewer-request tooling with the plugin tool and gh fallback.",
  );
});
