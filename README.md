...
The package also pins two Paperclip company settings in `.paperclip.yaml`: `attachmentMaxBytes: 10485760`, the 10 MiB company attachment cap introduced in `paperclipai@2026.428.0`, and `requireBoardApprovalForNewAgents: false`. `paperclipai@2026.831.1` retired the per-company attachment cap (the column was dropped and import strips the key), so the Paperclip process-level attachment cap is the final ceiling there; the key stays in the package for older runtimes and documents the intended limit. New-hire approval is opt-in in Paperclip, so this explicit `false` preserves the package's import behavior while still letting operators enable stricter hire approval in the live company settings before adding agents beyond the package roster.
