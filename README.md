...
When `GITHUB_TOKEN` is available, use `gh` for Micronaut organization-project lookup and PR project association. If `GITHUB_TOKEN` is not available, use `paperclip-github-plugin:list_organization_projects` and `paperclip-github-plugin:add_pull_request_to_project` so the live PR association exists in GitHub rather than only in comments or summaries. Here `GITHUB_TOKEN` means the environment variable with that exact name; do not search the filesystem, plugin config, or other files for a token.
