...
When `paperclip-github-plugin:create_pull_request` creates the PR, do not post the KPI webhook separately; the plugin records `pull_request_created` automatically. When an authenticated run creates a PR with `gh` or another non-plugin GitHub client in a repository mapped to the current company, immediately `POST /api/plugins/paperclip-github-plugin/webhooks/record-company-metric-event` with `metric: "pull_request_created"` plus either `pullRequestUrl` or `repository` and `pullRequestNumber`. Include `companyId` when the repository could map to more than one company. That request must be signed with the same company `GITHUB_TOKEN`: send `x-paperclip-github-sync-timestamp`, compute `x-paperclip-github-sync-signature` as `sha256=<hex-hmac>` over `${timestamp}.${rawBody}`, and use the exact JSON body you send as `rawBody`. Do not send this webhook for PR edits, comments, review replies, or merges. This is necessary because authenticated deployments currently cannot use the plugin tools directly, and GitHub alone cannot attribute those PRs to Paperclip work. This metric endpoint is a plugin webhook, not a plugin-tool call, so agents do not need to add an agent JWT or board-session header just for this request.
