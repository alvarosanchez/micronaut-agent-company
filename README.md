...
Recommended live routing is risk-classified by QA, not selected from the `type:` label alone. The stable `qa-intake` artifact records `deliveryClass`, `planningRequired`, `planningReason`, `securityPrecheckRequired`, `deliveryOwner`, `followThroughOwner`, `verificationProfile`, evidence/reproduction, and acceptance criteria.
