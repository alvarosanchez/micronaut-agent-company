...
All package-owned agents are configured to use `acpx_local` with Hermes ACP through the dedicated `paperclip` profile. The package pins the ACP command and non-interactive permission mode explicitly in `.paperclip.yaml` instead of relying on imported-instance adapter drift.

- Adapter type: `acpx_local`
- Command: `/usr/local/bin/hermes -p paperclip acp --accept-hooks`
- Mode: `persistent`
- Permission mode: `approve-all`
- Non-interactive permissions: `deny`
- Timeout: `0` (no adapter-level hard timeout)
- Grace period: `20` seconds

The config deliberately does **not** set deprecated adapter `cwd` or hardcoded `toolsets`. Paperclip should realize the project workspace for each run, and Hermes should use its default/all-tool behavior for the active `paperclip` profile. Live imported instances may add environment secret references such as `GITHUB_TOKEN`; those are instance-owned secrets and should be preserved during migrations rather than copied into this portable package.

The previous OpenCode cheap model profile is intentionally not configured for the Hermes ACP adapter defaults. The active model/provider are owned by the Hermes `paperclip` profile and can be changed there without reimporting this company package.
...
- This package models required gates as separate sequential stages. That is intentional: the installed `paperclipai@2026.517.0` runtime in this repository still exposes `approvalsNeeded: 1` for execution stages, so a single multi-participant stage should not be treated as a guaranteed unanimous gate.
...
When `paperclip-github-plugin:create_pull_request` creates the PR, do not post KPI attribution separately; the plugin records `pull_request_created` automatically. When an authenticated run creates a PR with `gh` or another non-plugin GitHub client in a repository mapped to the current company, immediately create the durable PR-to-Paperclip link with `POST /api/plugins/paperclip-github-plugin/api/issue-link` using `paperclipIssueId` plus `pullRequestUrl` or `reference`, then separately `POST /api/plugins/paperclip-github-plugin/api/company-metrics/events` with `metric: "pull_request_created"` plus either `pullRequestUrl` or `repository` and `pullRequestNumber`. Include `companyId` only when useful for disambiguation; if present, it must match the calling agent's company. GitHub alone cannot attribute non-plugin PR creation to Paperclip work, so both the durable link and the metric event are required. Authenticate both native plugin JSON routes with `Authorization: Bearer ${PAPERCLIP_API_KEY}`. The Paperclip host authenticates the bearer token, scopes each request to the calling agent's company, and rejects missing, expired, invalid, non-agent, or cross-company calls before the plugin worker handles it. This metric endpoint is a native plugin JSON route with agent auth, not a plugin-tool call or webhook. The issue-link endpoint uses the same native route authentication. Do not send the metric route call for PR edits, comments, review replies, or merges. The PR creation metric is not the issue link; confirm the `issue-link` route returns `status: "linked"` before reporting that GitHub Sync can track the PR.
...
Some workflow actions are Paperclip runtime concerns rather than GitHub sync concerns. In the current `paperclipai@2026.517.0` build, these are core APIs, not built-in agent-tool IDs:
...
When work involves visual or browser-rendered behavior, generated PDFs, reports, logs, or other reviewer-facing artifacts, agents should make the artifact PR-visible. QA records what was captured or generated and what each asset proves. Code Reviewer uploads assets with `paperclip-github-plugin:upload_pull_request_asset`, or with `POST /api/plugins/paperclip-github-plugin/api/pull-request-assets` when plugin tools are blocked in authenticated runs, then embeds the returned `asset.markdown` in the PR body. A local path, pasted base64 blob, or GitHub browser-only attachment flow is not durable evidence.
