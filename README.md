...
- Architect: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, reasoning effort `high`
- Security Engineer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, reasoning effort `high`
- QA Engineer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-terra`, reasoning effort `high`
- Code Reviewer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, reasoning effort `high`
- Product Manager: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-terra`, reasoning effort `high`
- CEO: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-terra`, reasoning effort `medium`
- Micronaut Engineer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, reasoning effort `high`
- Technical Writer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-luna`, reasoning effort `medium`

Each `hermes_local` adapter config sets `hermesCommand: /usr/local/bin/hermes-paperclip`, `provider: openai-codex`, a role-specific `model`, `extraArgs: [--reasoning-effort, <effort>]`, `persistSession: true`, `quiet: true`, `timeoutSec: 7200`, and `graceSec: 20`. The deployment-provided Hermes CLI wrapper must run the dedicated Hermes `paperclip` profile for `hermes chat` and may set deployment-specific routing such as `HERMES_CODEX_BASE_URL: http://headroom:8787/v1` before execing Hermes. On Cliponaut deployments, that wrapper also enables CodeGraph MCP by default through `/usr/local/bin/paperclip-codegraph-mcp`; set `PAPERCLIP_CODEGRAPH=off` only for an explicit deployment-level opt-out. CodeGraph resolves the active Paperclip project worktree and indexes it lazily with the deployment-pinned stable release; the same dedicated Hermes profiles must expose Paperclip plugin-provided tools through the deployment `paperclip-plugin-tools` MCP bridge so GitHub Sync tools appear in Hermes as `mcp_paperclip_plugin_tools_paperclip_github_plugin_*` schemas. The dedicated Hermes home mount itself remains deployment configuration; the Paperclip runtime must expose the `paperclip` Hermes profile at the path selected by the service environment. The adapters intentionally rely on Paperclip project workspaces, so they do not set `cwd`, and they do not pin `toolsets`; Hermes can load its default/all tool behavior for the dedicated profile.

Model-selection rationale: OpenAI's GPT-5.6 preview positions Sol as the flagship model and strongest model yet for agentic coding, long-horizon planning, biology, and cybersecurity, with a new `max` reasoning tier available for exceptional cases; Terra as a balanced everyday model with performance competitive to GPT-5.5 at half the API price; and Luna as the fast, affordable 5.6 tier. Published per-1M-token prices are GPT-5.6 Sol `$5 input / $30 output`, Terra `$2.50 input / $15 output`, and Luna `$1 input / $6 output`, compared with GPT-5.5 `$5 input / $30 output` and GPT-5.5 Pro `$30 input / $180 output`. Therefore this package spends Sol only on roles where stronger coding/security/review reasoning directly reduces expensive downstream failures, uses Terra for broad coordination, triage, and research work that previously used GPT-5.5, and uses Luna for documentation writing where speed and output cost matter and the review pipeline still catches risky changes. The package deliberately avoids default `max` effort to prevent routine runs from over-spending; high/medium are the steady-state defaults, with one-off max/ultra use reserved for manual escalation outside the portable package.

The agent instructions add a concise operating profile for each model family: Sol roles form and test explicit code/security hypotheses and use CodeGraph for call-path analysis; Terra roles batch independent retrieval and reduce it to decision tables before deeper work; Luna transforms verified evidence into concise documentation with bounded checks. Common GitHub transport and execution-policy rules live in shared skills rather than being copied into every role. The always-loaded `micronaut-repo-operations` entrypoint is a compact router: it keeps the cross-role stage contract and sends release, GitHub, routine, recovery, and PR-detail procedures to imported on-demand references. Its `cheap` model boundary limits `gpt-5.4-mini` to classification, bounded reads, adapter checks, and deterministic no-op detection; substantive decisions and external writes must use the role's primary model. The skill also ships `scripts/repo-evidence.mjs`, a read-only collector that returns one compact JSON object containing repository root, branch, HEAD, upstream, worktree status, optional base divergence, changed files, build markers, and instruction files. Agents should run it once with `--base <approved-ref>` instead of reconstructing the same local preflight through many shell calls.

Toolsets remain unpinned in this portable package because CodeGraph and GitHub Sync arrive through deployment-owned MCP server names. An operator may narrow the wrapper's toolsets only after preserving both MCP surfaces and validating representative intake, implementation, review, and PR-publication workflows; otherwise schema savings would come at the cost of required capability.
...
- It does include one lightweight internal project, `company-operations`, whose bootstrap CEO verification task imports on the CEO queue and whose seven recurring tasks import as active Paperclip routines, including frequent incident containment plus six monthly maintenance routines.
...
| `Training` | CEO | 25th of each month at 02:00 `Europe/Madrid` | Analyze non-CEO agent executions since the last Training pass for recurring technology, domain, stack, and tool gaps, find targeted https://skills.sh skills, request board approval for each external candidate, add approved referenced company skills to the approved agents, or create a `backlog` Architect subtask when a new company skill PR is warranted |
| `Monthly CEO Self-Improvement` | CEO | 20th of each month at 03:00 `Europe/Madrid` | Run the deterministic 30-day issue-history collector, rank only threshold-backed issue-level evidence, fail closed on incomplete coverage, then handle skill sync, managed-repository guidance audit, and prior CEO PR follow-up as separate maintenance lanes |
...
| `paperclipai/bundled/software-development/github-pr-workflow` | CEO, Code Reviewer, Micronaut Engineer, Technical Writer | Branch hygiene, PR body evidence, review-thread replies, CI/readiness checks, and PR follow-through while preserving the normal Code Reviewer PR-creation gate |
