...
Paperclip v2026.626.0 also adds built-in Hermes adapters (`hermes_local` and `hermes_gateway`). This package intentionally keeps the existing `acpx_local` custom-command contract for now because it is deployment-provided and already routes through `/usr/local/bin/hermes-paperclip-acp`; switch to the built-in Hermes adapter only after import/export round-trip verification preserves the required model, permission, and gateway settings. Task watchdogs are available for non-GitHub waits, but GitHub Sync remains the source of truth for PR CI, mergeability, review-thread, and linked-PR state. Ask work mode is appropriate for explicit question-and-answer tasks, not normal delivery pipeline items. Routine date variables can be adopted by future routine prompt updates without changing the active overnight cadence. Teams Catalog is a runtime catalog surface; this package should reference catalog teams/skills by key instead of vendoring them.
