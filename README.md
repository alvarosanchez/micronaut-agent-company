...
- Plugin tools: `GET /api/plugins/tools` lists the plugin tools the tool gateway allows for this agent, and `POST /api/plugins/tools/execute` runs one by its `<pluginId>:<tool>` id with the run context; both use the run token.
...
7. Importing the bootstrap issue does not automatically wake the CEO. An authenticated human operator may use the UI's `Review now` action for `Verify Imported Company Instance`; package agents must not invoke another agent's heartbeat.
