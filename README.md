...
- Decisions (`POST /api/companies/{companyId}/decisions` propose/decide, queues, desk), Cases, status cards, and summary slots are deliberately not used by this package in favour of linked approvals plus `request_confirmation`: Cases, status cards, and summary slots are experimental and flag-off by default, and a resolved Decision wakes only the origin agent and carries no `recommendedAction` field, so it fits this workflow worse than the approval pattern already in place. Treat their absence as a decision, not as a gap to fill.
