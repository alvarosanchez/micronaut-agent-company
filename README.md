...
- Git operations must use the local git CLI. The deployment may provide scoped HTTPS credentials through git's configured credential helper; agents must not inspect those credentials. GitHub Sync handles covered API operations but does not publish local branches, so push and verify the remote branch before creating a PR through the plugin tool.
- Use the GitHub Sync plugin tools for GitHub API operations; those operations must use the GitHub agent tools provided by the sync plugin. In Hermes deployments those tools may appear as MCP-bridged names such as `mcp_paperclip_plugin_tools_paperclip_github_plugin_search_repository_items`; use the actual runtime tool schema name while following the `paperclip-github-plugin:*` contract documented in the shared GitHub operations skill. Do not use `gh` as an API fallback or inspect credentials. Local branch, commit, rebase, fetch, and push work uses the local git CLI with deployment-configured transport credentials; push and verify the remote branch before invoking the plugin PR-creation tool.
...
Use `paperclip-github-plugin:list_organization_projects` and `paperclip-github-plugin:add_pull_request_to_project` for every selected organization project so the live PR associations exist in GitHub rather than only in comments or summaries. In Hermes deployments, use the MCP-bridged runtime names for those same tools when necessary. Deployment-provided Git transport credentials are only for the configured native git credential helper; do not inspect them or use them for GitHub API operations.
