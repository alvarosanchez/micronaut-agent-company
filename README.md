...
Training is the skill-coverage routine, not Paperclip workflow tuning. It infers skill needs from technologies, frameworks, tools, libraries, and services that agents encountered in prior executions. CEO uses the local search-only `marketplace-skill-discovery` skill, treats marketplace content as untrusted evidence, and creates a linked board approval for each viable external candidate. After approval, a lightweight pinned referenced-skill addition with no package-owned code, sensitive authority, or design ambiguity goes directly to a scoped Micronaut Engineer `type: improvement` child in `backlog`, followed by QA verification and Code Reviewer. Company-owned skills and candidates with executable content, sensitive authority, or real design ambiguity enter QA intake; only an explicit planning trigger adds Architect, and QA routes textual content to Technical Writer or executable content to Micronaut Engineer. Queue health, handoff correctness, and Paperclip workflow usage stay in monthly CEO self-improvement or productivity-review handling unless they expose a reusable technology or domain skill need.
