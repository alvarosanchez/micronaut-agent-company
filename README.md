...
- Plugin tools: `GET /api/plugins/tools` lists the plugin tools the tool gateway allows for this agent, and `POST /api/plugins/tools/execute` runs one by its `<pluginId>:<tool>` id with the run context; both use the run token. The gateway is fail-closed, so an empty list or a `deny_default` execute rejection means no active tool-access profile bound to this agent includes the tool; report that as a deployment gap instead of retrying or falling back.
