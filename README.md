...
- Architect: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, high reasoning
- QA Engineer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, high reasoning
- Security Engineer: `hermes_local` via `/usr/local/bin/hermes-paperclip`, `gpt-5.6-sol`, high reasoning
...
Model selection follows role exposure: Sol/high for architecture, authoritative QA classification, security, and final review; Terra/high for engineering and textual control-plane delivery; Terra/medium for governance and product discovery. Reasoning effort is pinned per role through the deployment wrapper contract rather than left implicit.

The always-loaded `micronaut-repo-operations` entrypoint centralizes workflow mechanics. Its `cheap` boundary limits `gpt-5.4-mini` to bounded read-only work. The deployment's optional CodeGraph MCP at `/usr/local/bin/paperclip-codegraph-mcp` does not require agents to use it for every coding task; it remains useful for unfamiliar or large repositories, cross-module call paths, or repetitive symbol discovery, but is unnecessary for localized fixes and documentation/configuration work.

Toolsets remain unpinned in this portable package because deployment-owned MCP server names may provide GitHub Sync and optional tools such as CodeGraph. An operator may narrow the wrapper's toolsets only after preserving the MCP surfaces its workflows rely on and validating representative intake, implementation, review, and PR-publication workflows.

Each agent also configures Paperclip's cheap model profile in `.paperclip.yaml` with `runtime.modelProfiles.cheap.enabled: true`, `provider: openai-codex`, and `model: gpt-5.4-mini`. The primary built-in Hermes adapters now use role-specific GPT-5.6 models; the cheap profile remains on `gpt-5.4-mini` because it is still cheaper than Luna and is intended only for low-cost Paperclip wakeups, adapter config checks, or orchestration paths that explicitly request it.

Paperclip v2026.609.0 raised the default agent heartbeat concurrency to 20 concurrent runs per agent; this remains true for the current `paperclipai@2026.626.0` runtime. This package deliberately overrides that runtime default for every package-owned agent with `runtime.heartbeat.maxConcurrentRuns: 1` in `.paperclip.yaml` while the Micronaut workflow is tuned for one owned work item per agent at a time. Operators can raise that value in a live company later when the queue and machine capacity are ready for wider parallelism.
...
Paperclip v2026.512.0, still true in the current `paperclipai@2026.626.0` runtime, also carries issue `workMode` and applies status defaults during issue creation: assigned issue status defaults to `todo` when status is omitted, while unassigned issues default to `backlog`; an explicit `backlog` status still wins. This company's normal delivery issues, routine child issues, PR follow-through issues, and Product Manager-created product development issues use standard work mode. Do not set `workMode: planning` for the normal Architect planning stage, because those delivery issues must continue into implementation after the plan. Planning mode is for planning-only precursor issues created by CEO, Product Manager, or Architect when a human explicitly wants a plan, scope breakdown, rollout design, or child-task graph before implementation. A planning-mode owner writes or updates the `plan` issue document only, does not write code or start implementation there, asks for confirmation with a `request_confirmation` interaction when approval is needed, and after acceptance calls `POST /api/issues/{issueId}/accepted-plan-decompositions` so Paperclip creates idempotent child implementation issues with `workMode: standard`.

Paperclip v2026.626.0 adds the Skills Store, workspace file viewer/artifact links, and self-hostable sandbox execution, plus task watchdogs, ask work mode, routine date variables, Teams Catalog, workspace file downloads, and built-in Hermes local/gateway adapters. This package now grants selected Paperclip catalog skill keys in agent frontmatter without vendoring the catalog skill bodies; operators must install those Skills Store entries in the target company so the preserved grants become configured runtime skills.

Paperclip v2026.626.0 also adds built-in Hermes adapters (`hermes_local` and `hermes_gateway`). This package adopts `hermes_local` for package-owned agents, while deliberately not adopting `hermes_gateway` yet because Cliponaut currently runs Hermes in the Paperclip server container rather than as a separate long-lived Hermes API server. A future gateway migration should be a deployment PR that provisions `API_SERVER_ENABLED=true`, `API_SERVER_KEY`, a private/TLS `apiBaseUrl`, and `paperclipApiUrl`, then updates the package adapter config after staging run verification. Task watchdogs are available for non-GitHub waits, but GitHub Sync remains the source of truth for PR CI, mergeability, review-thread, and linked-PR state. Ask work mode is appropriate for explicit question-and-answer tasks, not normal delivery pipeline items. Routine date variables can be adopted by future routine prompt updates without changing the active overnight cadence. Teams Catalog is a runtime catalog surface; this package should reference catalog teams/skills by key instead of vendoring them.

Agents must still audit the imported runtime skill inventory and the dedicated Hermes profile skill visibility before assuming a skill is usable. Visual evidence, generated reports, and browser artifacts should prefer Paperclip workspace file viewer or PR-visible artifact links over local-only paths. Sandbox-backed environments remain live deployment configuration: choose local, SSH-backed, Kubernetes, E2B, Novita, or other environment-driver plugins in the Paperclip instance, not in this portable package.
...
- This package models required gates as separate sequential stages. That is intentional: the installed `paperclipai@2026.626.0` runtime in this repository still exposes `approvalsNeeded: 1` for execution stages, so a single multi-participant stage should not be treated as a guaranteed unanimous gate.
...
- Use the GitHub Sync plugin tools for GitHub API operations; those operations must use the GitHub agent tools provided by the sync plugin. In Hermes deployments those tools may appear as MCP-bridged names such as `mcp_paperclip_plugin_tools_paperclip_github_plugin_search_repository_items`; use the actual runtime tool schema name while following the `paperclip-github-plugin:*` contract documented in the shared GitHub operations skill. Do not use `gh` as an API fallback, inspect credentials, or publish branches outside the atomic `create_pull_request` tool.
...
- When those selected organization projects exist and GitHub tooling can apply them, agents should use the GitHub Sync project-link tool so the PR carries every association instead of only naming the intended boards in a comment or PR description; use `paperclip-github-plugin:add_pull_request_to_project` or its MCP-bridged runtime name.
...
Some workflow actions are Paperclip runtime concerns rather than GitHub sync concerns. In the current `paperclipai@2026.626.0` build, these are core APIs, not built-in agent-tool IDs:
...
These are Paperclip 2026.626 Skills Store catalog prerequisites, not vendored source-package skills. This package grants agents the catalog skill **keys** so the assignment survives import/export without duplicating upstream skill bodies. On a fresh imported company, install the catalog skills from the Skills Store; before installation Paperclip preserves the desired grants but reports those runtime skills as missing. Micronaut-specific corrections live in the relevant agent instructions below rather than in copied catalog skill bodies.
