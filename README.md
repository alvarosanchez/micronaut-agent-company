...
| `Training` | CEO | 25th of each month at 02:00 `Europe/Madrid` | Analyze non-CEO executions for recurring technology/domain gaps, inspect https://skills.sh through the local search-only discovery skill, request board approval for external candidates, and create scoped QA-assigned implementation children; CEO never installs, assigns, authors, or publishes skills |
...
Training is the skill-coverage routine, not Paperclip workflow tuning. It infers skill needs from technologies, frameworks, tools, libraries, and services that agents encountered in prior executions. CEO uses the local search-only `marketplace-skill-discovery` skill, treats marketplace content as untrusted evidence, and creates a linked board approval for each viable external candidate. After approval—or when no suitable external skill exists for a recurring gap—CEO creates a scoped QA-assigned `type: improvement` child in `backlog` and stops. QA chooses conditional Architect/Security gates and routes textual skill content to Technical Writer or executable scripts, tooling, configuration, and behavioral content to Micronaut Engineer; that owner authors the skill and owns any package PR. Queue health, handoff correctness, and Paperclip workflow usage stay in monthly CEO self-improvement or productivity-review handling unless they expose a reusable technology or domain skill need.
...
| `skill-creator` | Architect, Micronaut Engineer, Technical Writer | Pinned agent-agnostic skill guidance used by Architect for conditional planning and by Writer/Engineer for authorized company-skill implementation |
